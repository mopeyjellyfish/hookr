...
func generateFlatBuffers(cfg Config) error {
	...
	case "js", "javascript", "ts", "typescript", "javy":
		return generateFlatBuffersJavaScript(cfg)
	default:
		return errors.New("flatbuffers generation currently supports only --lang go, --lang rust, --lang zig, --lang cpp, --lang assemblyscript, --lang c, --lang swift, or --lang js")
	}
}
...
func generateFlatBuffersJavaScript(cfg Config) error {
	runner, err := flatc.New(cfg.FlatcPath)
	if err != nil {
		return err
	}
	...
	packageDir := filepath.Join(cfg.OutDir, cfg.PackageName)
	if err := os.MkdirAll(packageDir, 0o755); err != nil {
		return fmt.Errorf("create javascript output dir: %w", err)
	}
	if err := runner.GenerateTypeScript(flatc.TypeScriptOptions{
		SchemaPath:   cfg.SchemaPath,
		OutDir:       packageDir,
		IncludePaths: cfg.IncludePaths,
	}); err != nil {
	...
	if err := renderFlatbuffersTemplate(filepath.Join(packageDir, "hookr_plugin.ts"), flatbuffersJavaScriptPluginTemplate, td); err != nil {
...
const flatbuffersJavaScriptPluginTemplate = `// Code generated by hookr. DO NOT EDIT.

export const ABI_MAJOR = 2;
export const ABI_MINOR = 0;
export const CONTRACT_CAPABILITIES = BigInt("{{ .ContractCapabilities }}");
export const SCHEMA_HASH = new Uint8Array([
  {{ join .SchemaHashLiterals ", " }}
]);

{{ range .PluginMethods -}}
export const {{ .RustConstName }} = {{ .ID }};
{{ end }}
{{ if .HasHostModules }}
{{ range .HostModules }}
{{ range .Methods -}}
...
export type HookrDispatch =
  (methodId: number, payload: Uint8Array) => Uint8Array | Promise<Uint8Array>;

export interface HookrHostBridge {
  pluginRequest(payloadLen: number): Uint8Array;
  pluginResponse(payload: Uint8Array): void;
...
let dispatchFn: HookrDispatch | undefined;
let methodBytes = defaultImplementedMethodBytes();

export function registerPlugin(dispatch: HookrDispatch, implementedMethodBytes?: Uint8Array): void {
  dispatchFn = dispatch;
  methodBytes = implementedMethodBytes ?? defaultImplementedMethodBytes();
}
...
export async function pluginCall(
  host: HookrHostBridge,
  methodId: number,
  payloadLen: number,
): Promise<boolean> {
  const dispatch = dispatchFn;
  if (dispatch === undefined) {
    host.pluginError("plugin implementation is not registered");
    return false;
  }
  try {
    const payload = host.pluginRequest(payloadLen);
    const response = await dispatch(methodId >>> 0, payload);
    host.pluginResponse(response);
    return true;
  } catch (error) {
    host.pluginError(error instanceof Error ? error.message : String(error));
    return false;
  }
}

export function hostCallRaw(host: HookrHostBridge, methodId: number, payload: Uint8Array): Uint8Array {
  return host.hostCall(methodId >>> 0, payload);
}

export function log(host: HookrHostBridge, message: string): void {
  host.log(message);
}

export function abiVersion(): number {
  return ((ABI_MAJOR & 0xffff) << 16) | (ABI_MINOR & 0xffff);
}

export function schemaHash(): Uint8Array {
  return SCHEMA_HASH;
}

export function capabilities(): bigint {
  return CONTRACT_CAPABILITIES;
}

export function implementedMethods(): Uint8Array {
  return methodBytes;
}
